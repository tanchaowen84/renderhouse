...
### 实施阶段（每阶段可在前端手动验收）
1) 聊天流骨架（假数据）：左侧文本输入+消息列表+“渲染完成”卡片，使用本地 mock 驱动；右侧画布可被假图替换。
2) 接入真实 AI 对话（先不调 fal）：`/api/chat` 接入 OpenRouter Grok‑4.1‑fast，返回“自然语言回复 + renderSpec(prompt 草案等)”的结构化结果；不生成图片，仅验证个性化回复、上下文承接与 prompt 草案输出。
3) 打通 fal 渲染覆盖（落地 R2）：`/api/chat` 工具调用 Seedream → 后端拉取结果图并上传到 Cloudflare R2 → 覆盖 DB 中最新渲染 URL（仅存 R2 链接，不存文件）；前端替换画布与卡片，刷新后仍显示最新版本，旧图在新图成功后删除。
4) 失败与并发守卫：渲染中禁用输入，失败 toast 且不覆盖旧图；后端同项目加锁/超时处理；连续点击只跑一单。
5) 体验打磨：状态 badge、下载按钮、聊天历史截断/恢复，空态清晰；重新进入工作台能看到最新图和关键记录。
