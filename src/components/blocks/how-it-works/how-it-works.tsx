import { HeaderSection } from '@/components/layout/header-section';
import { Button } from '@/components/ui/button';
import { LocaleLink } from '@/i18n/navigation';
import { ChevronRight } from 'lucide-react';
import { useTranslations } from 'next-intl';
import Image from 'next/image';
import type * as React from 'react';

export default function HowItWorksSection() {
  const t = useTranslations('HomePage.howItWorks');

  return (
    <section id="how-it-works" className="px-4 py-16">
      <div className="mx-auto max-w-6xl space-y-8 lg:space-y-16">
        <HeaderSection
          title={t('title')}
          subtitle={t('subtitle')}
          description={t('description')}
          subtitleAs="h2"
          descriptionAs="p"
        />

        <div className="grid items-start gap-10 lg:grid-cols-5 lg:gap-16">
          <div className="lg:col-span-2">
            <ul className="divide-y divide-[#e6e8ec] border-y border-[#e6e8ec] *:flex *:items-start *:gap-4 *:py-4">
              <li>
                <div className="mt-1 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-[#d9dde1] bg-white text-sm font-semibold text-[#1f4b3e]">
                  1
                </div>
                <div className="space-y-2">
                  <h3 className="font-semibold text-[#1f242c]">
                    {t('steps.step-1.title')}
                  </h3>
                  <p className="text-sm leading-relaxed text-[#6a707a]">
                    {t('steps.step-1.description')}
                  </p>
                  <ul className="space-y-1 text-xs text-[#7a7f87]">
                    {(t.raw('steps.step-1.details') as string[]).map(
                      (detail, index) => (
                        <li key={index} className="flex items-start gap-2">
                          <span className="text-[#1f4b3e]">•</span>
                          <span>{detail}</span>
                        </li>
                      )
                    )}
                  </ul>
                </div>
              </li>
              <li>
                <div className="mt-1 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-[#d9dde1] bg-white text-sm font-semibold text-[#1f4b3e]">
                  2
                </div>
                <div className="space-y-2">
                  <h3 className="font-semibold text-[#1f242c]">
                    {t('steps.step-2.title')}
                  </h3>
                  <p className="text-sm leading-relaxed text-[#6a707a]">
                    {t('steps.step-2.description')}
                  </p>
                  <ul className="space-y-1 text-xs text-[#7a7f87]">
                    {(t.raw('steps.step-2.details') as string[]).map(
                      (detail, index) => (
                        <li key={index} className="flex items-start gap-2">
                          <span className="text-[#1f4b3e]">•</span>
                          <span>{detail}</span>
                        </li>
                      )
                    )}
                  </ul>
                </div>
              </li>
              <li>
                <div className="mt-1 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full border border-[#d9dde1] bg-white text-sm font-semibold text-[#1f4b3e]">
                  3
                </div>
                <div className="space-y-2">
                  <h3 className="font-semibold text-[#1f242c]">
                    {t('steps.step-3.title')}
                  </h3>
                  <p className="text-sm leading-relaxed text-[#6a707a]">
                    {t('steps.step-3.description')}
                  </p>
                  <ul className="space-y-1 text-xs text-[#7a7f87]">
                    {(t.raw('steps.step-3.details') as string[]).map(
                      (detail, index) => (
                        <li key={index} className="flex items-start gap-2">
                          <span className="text-[#1f4b3e]">•</span>
                          <span>{detail}</span>
                        </li>
                      )
                    )}
                  </ul>
                </div>
              </li>
            </ul>
          </div>

          <div className="relative lg:col-span-3">
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-[#e3e6ea] bg-white shadow-[0_18px_40px_rgba(0,0,0,0.08)]">
              <Image
                src="https://cdn.flowchartai.org/static/blocks/howitworks1.png"
                className="h-full w-full object-cover object-center"
                alt="How FlowChart AI Works - Step by Step Process"
                width={800}
                height={600}
              />
            </div>
          </div>
        </div>

        <div className="mt-12 text-center">
          <Button
            asChild
            size="lg"
            className="gap-2 rounded-full border border-[#1f4b3e] bg-[#1f4b3e] text-white hover:brightness-[1.05]"
          >
            <LocaleLink href="/canvas">
              {t('getStarted')}
              <ChevronRight className="!size-4" />
            </LocaleLink>
          </Button>
        </div>
      </div>
    </section>
  );
}
