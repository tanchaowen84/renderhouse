...
export function WorkspaceClient({ initialRecord, messages }: WorkspaceClientProps) {
  ...
  return (
    <>
      ...
      <div className="absolute left-6 top-6 bottom-6 z-30 w-[400px] flex flex-col">
            ...
            {step === 'initial' && (
              ...
              >
                <div className="mb-4 flex items-center gap-2 text-slate-100">
                  ...
                  <h2 className="text-lg font-bold">{messages.actions.title}</h2>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <button className="group w-full rounded-2xl border border-white/5 bg-white/5 px-5 py-5 text-left transition-all hover:bg-white/10 hover:border-white/10 active:scale-[0.98]">
            ...
            {step === 'settings' && (
              ...
              >
                ...
                <div className="flex-1 overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
                  ...
                  {/* Location Section */}
                  <div className="mb-8">
                    <h3 className="mb-3 text-sm font-semibold text-slate-300">Location</h3>
                    <p className="mb-3 text-xs text-slate-500">
                      Select the location for your render to get accurate lighting
                    </p>
                    <button className="flex w-full items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 text-left text-sm text-slate-400 hover:bg-white/10 hover:text-white transition-colors">
                  ...
                  {/* Lighting Section */}
                  <div className="mb-8">
                    <h3 className="mb-3 text-sm font-semibold text-slate-300">Lighting</h3>
                    <p className="mb-3 text-xs text-slate-500">
                      Choose the time of day for your exterior render
                    </p>
                    <div className="grid grid-cols-3 gap-3">
                      {[
                        { id: 'sunrise', label: 'Sunrise', icon: SunriseIcon, color: 'text-orange-400', bg: 'bg-orange-400/20' },
                        { id: 'morning', label: 'Morning', icon: CloudSunIcon, color: 'text-sky-400', bg: 'bg-sky-400/20' },
                        { id: 'noon', label: 'Noon', icon: SunIcon, color: 'text-yellow-400', bg: 'bg-yellow-400/20' },
                        { id: 'afternoon', label: 'Afternoon', icon: SunIcon, color: 'text-amber-400', bg: 'bg-amber-400/20' },
                        { id: 'sunset', label: 'Sunset', icon: SunsetIcon, color: 'text-rose-400', bg: 'bg-rose-400/20' },
                        { id: 'night', label: 'Night', icon: MoonIcon, color: 'text-indigo-400', bg: 'bg-indigo-400/20' },
                      ].map((item) => (
                        <button
                          key={item.id}
                        ...
                        >
                          <div className={cn("flex h-10 w-10 items-center justify-center rounded-full", item.bg, item.color)}>
                            <item.icon className="size-5" />
                          </div>
                          <span className={cn("text-xs font-medium", selectedLighting === item.id ? "text-white" : "text-slate-400")}>
                            {item.label}
                          </span>
                        </button>
                      ))}
                  ...
                  <div className="mb-8">
                    <div className="flex items-center justify-between mb-3">
                      <h3 className="text-sm font-semibold text-slate-300">Camera Angle</h3>
                    </div>
                    <div 
                    ...
                    >
                      <div className={cn("flex h-5 w-5 items-center justify-center rounded border transition-colors", maintainAngle ? "bg-indigo-500 border-indigo-500" : "border-slate-500")}>
                        {maintainAngle && <div className="h-2 w-2 rounded-[1px] bg-white" />}
                      </div>
                      <span className="text-sm text-slate-300">Maintain angle</span>
                    </div>
                  </div>

                  {/* More Details */}
                  <div className="mb-4">
                    <h3 className="mb-3 text-sm font-semibold text-slate-300">More Details</h3>
                    <textarea 
      ...
      {/* Image Stage (Center) - Adjusted padding for wider sidebar */}
      <div className="flex h-full w-full items-center justify-center p-8 pl-[420px] lg:pl-[440px] animate-in zoom-in-95 fade-in duration-700">
